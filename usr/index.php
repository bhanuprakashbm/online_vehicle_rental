...
<?php
    ...
    if(isset($_POST['Usr-login']))
    {
      ...
      $_SESSION['login']=$u_email; // Changed $uemail to $u_email
      ...
      {//get user logs
        ...
        // Try to log the login attempt, but don't fail if the table doesn't exist
        try {
            $log = $mysqli->prepare("INSERT INTO userLog(u_id, u_email, u_ip, u_city, u_country) VALUES (?, ?, ?, ?, ?)");
            if ($log) {
